...
ARS 最初是为 Claude Code 开发的插件（[imbad0202/academic-research-skills](https://github.com/Imbad0202/academic-research-skills) v3.9.4.2），现已完整移植到 **OpenCode** 平台（版本号 `3.9.4.2-oc`）。
...
ARS 提供了 **13 条**预配置的快捷命令，覆盖最常用的科研场景（见 @tbl-commands）。
...
ARS 共提供 **13 条**命令，按功能分为四组：研究、写作、评审、管线。

: 13 条命令速查 {#tbl-commands}
...
> **一致性说明**：原 DOCX 版本仅列出 9 条命令（遗漏 `/ars-socratic`、`/ars-systematic-review`、`/ars-reviewer`、`/ars-calibrate`），且未提供具体使用示例。本版本已补全全部 13 条，并每一条配有真实使用场景。
...
## ars-academic-paper — 学术论文（10 种模式）
...
1. **命令数量校验**：`opencode.json` 中定义的 13 条命令，在上文 @tbl-commands 中全部列出（13/13 条），无遗漏
